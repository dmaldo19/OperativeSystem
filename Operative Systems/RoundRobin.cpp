...
void impresionesDeProcesos(queue<proceso>& total, queue<proceso>& espera, queue<proceso>& final, int& contador, queue<proceso>& procesosBloqueados, int& id, int quantum);
...
int main(){
    ...
    do
    {
        ...
        while (!EnEspera.empty() || !Bloqueados.empty()){
            system("cls");
            impresionesDeProcesos(colaProcesos, EnEspera, Terminado, tiempo, Bloqueados, id, quantum); //Invocamos funcion para mostrarlos
