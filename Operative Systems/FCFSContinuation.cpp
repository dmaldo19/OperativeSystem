...
void impresionesDeProcesos(queue<proceso>& total, queue<proceso>& espera, queue<proceso>& final, int& contador, queue<proceso>& procesosBloqueados, int& id);
...
int main(){
    ...
    queue<proceso> colaProcesos, colaTerminadosParcial; //cola principal que almacenara todos los procesos, junto con la cola para mostrar la tabla momentanea
    gotoxy(0, 0); cout << "-----Programa de simulacion FCFS continuacion-----";
    ...
    do
    {
        ...
        while (!EnEspera.empty() || !Bloqueados.empty()){
            system("cls");
            impresionesDeProcesos(colaProcesos, EnEspera, Terminado, tiempo, Bloqueados, id); //Invocamos funcion para mostrarlos
        ...
        if(kbhit()){
            ...
            switch (key)
            {
                ...
                case 'n':
                    ...
                    if(espera.size()+procesosBloqueados.size() < 4){
